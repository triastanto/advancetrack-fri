...
trait HasDocumentManagement
{
     ...
     */
    public function getDocumentCompletionStatus($employee, $documentTypeIds, bool $useCache = true): array
    {
        ...
        if ($useCache) {
            return $this->getCachedCompletionStatus($employee, $documentTypeIds);
        }

        return $this->calculateCompletionStatus($employee, $documentTypeIds);
    }

    /**
     * Get cached completion status
     */
    protected function getCachedCompletionStatus($employee, $documentTypeIds): array
    {
        $cacheKey = $this->generateCompletionStatusCacheKey($employee, $documentTypeIds);
        
        return Cache::remember($cacheKey, now()->addMinutes(self::CACHE_TTL_MINUTES), function () use ($employee, $documentTypeIds) {
            return $this->calculateCompletionStatus($employee, $documentTypeIds);
        });
    }
     ...
     */
    protected function generateCompletionStatusCacheKey($employee, $documentTypeIds): string
    {
        $sortedIds = $documentTypeIds;
        sort($sortedIds);
        return self::COMPLETION_STATUS_CACHE_PREFIX . "_{$employee->id}_" . md5(implode(',', $sortedIds));
    }

    /**
     * Calculate completion status without caching
     */
    protected function calculateCompletionStatus($employee, $documentTypeIds): array
    {
        try {
            $uploadedDocuments = $this->getDocumentsWithOptimizedQueries($employee, $documentTypeIds);
     ...
     */
    protected function getDocumentsWithOptimizedQueries($employee, $documentTypeIds, $withRelations = [])
    {
