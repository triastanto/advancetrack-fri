    ...
    <aside id="sidebar" x-data="{ userMenuOpen: false }" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="flex flex-col h-screen w-72 bg-[var(--color-bg)] border-r border-[var(--color-border)] fixed top-0 left-0 z-40 transform lg:translate-x-0 transition-transform duration-200 ease-in-out lg:static lg:flex">
        ...
        <nav class="flex-1 px-4">
            <ul class="space-y-2 mt-4">
                ...
                <!-- Data Pribadi Link -->
                <li>
                    <a href="{{ route('lecturer.profile') ?? '/profile' }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm {{ request()->is('profile*') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">
                ...
                <li x-data="{ submenuOpen: true }" class="relative">
                    <div class="flex items-center justify-between w-full gap-3 px-3 py-2 mb-1 rounded-lg cursor-pointer text-sm {{ request()->routeIs('documents.*') ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-main)]' }}" @click="submenuOpen = !submenuOpen">
                        <div class="flex items-center gap-3">
                            <span class="material-icons-outlined">folder</span>
                            <span class="{{ request()->routeIs('documents.*') ? 'font-semibold' : '' }}">Dokumen Saya</span>
                        </div>
                    ...
                    <div x-show="submenuOpen" x-transition class="pl-6 mt-1 space-y-1">
                        <a href="{{ route('documents.study-requirements') }}" class="block py-2 px-3 rounded-lg text-sm {{ request()->routeIs('documents.study-requirements') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">Kelengkapan Studi Lanjut</a>
                        <a href="{{ route('documents.semester-reports') }}" class="block py-2 px-3 rounded-lg text-sm {{ request()->routeIs('documents.semester-reports') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">Laporan Per Semester</a>
                        <a href="{{ route('documents.final-reports') }}" class="block py-2 px-3 rounded-lg text-sm {{ request()->routeIs('documents.final-reports') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">Laporan Akhir & Kelulusan</a>
                        <a href="{{ route('documents.additional') }}" class="block py-2 px-3 rounded-lg text-sm {{ request()->routeIs('documents.additional') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">Dokumen Tambahan</a>
                        <a href="{{ route('documents.service-bond') }}" class="block py-2 px-3 rounded-lg text-sm {{ request()->routeIs('documents.service-bond') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">Perjanjian Ikatan Dinas</a>
                    </div>
                </li>
                <!-- Administrasi Dokumen Group -->
                <li x-data="{ adminSubmenuOpen: true }" class="relative">
                    <div class="flex items-center justify-between w-full gap-3 px-3 py-2 mb-1 rounded-lg cursor-pointer text-sm {{ request()->routeIs('admin.documents.*') || request()->routeIs('verification') ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-main)]' }}" @click="adminSubmenuOpen = !adminSubmenuOpen">
                        <div class="flex items-center gap-3">
                            <span class="material-icons-outlined">inventory_2</span>
                            <span class="{{ request()->routeIs('admin.documents.*') || request()->routeIs('verification') ? 'font-semibold' : '' }}">Administrasi Dokumen</span>
                        </div>
                    ...
                    <div x-show="adminSubmenuOpen" x-transition class="pl-6 mt-1 space-y-1">
                        <a href="{{ route('admin.documents.lecturers') }}" class="block py-2 px-3 rounded-lg text-sm {{ request()->routeIs('admin.documents.lecturers') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">Cari & Pilih Dosen</a>
                        <a href="{{ route('admin.documents.upload') }}" class="block py-2 px-3 rounded-lg text-sm {{ request()->routeIs('admin.documents.upload') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">Unggah Dokumen</a>
                        <a href="{{ route('verification') }}" class="block py-2 px-3 rounded-lg text-sm {{ request()->routeIs('verification') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">Verifikasi Dokumen</a>
                    </div>
                </li>
                <!-- Monitoring & Laporan Link -->
                <li>
                    <a href="/reports" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm {{ request()->is('reports*') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">
                ...
                <!-- Pengaturan Link -->
                <li>
                    <a href="/settings" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm {{ request()->is('settings*') ? 'text-[var(--color-primary)] bg-[var(--color-primary-bg)] font-semibold' : 'text-[var(--color-text-main)] hover:bg-[var(--color-primary-bg)]' }}">
